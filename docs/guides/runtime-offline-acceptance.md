...
范围说明：脚本使用临时二进制和合成数据，验证当前源码的迁移、恢复、Owner Assistant 任务与发送审计；完整 Agent、机器人权限及上下文测试另由 `make check` 和 `make test-runtime-race` 执行。[验收矩阵](../architecture/best-practice-scenarios-detail.md#后台观察与机器人交互验收)分别记录代码、二进制与真实平台结果，历史结果不能代替本轮验证。
...
- 使用单一 `~/.memgov/config.yaml` 做配置计划，检查 Owner 身份、授权边界、无 unmanaged conflict、无重复 runtime 和无未授权权限扩张；`config.dual.yaml` 仅作为迁移备份。
- Owner 私聊直接回答和复杂任务委派，检查根任务、子 Agent、上下文来源、权限版本、证据和通知幂等记录；proactive 只在有实质结果、阻塞或确认请求时通知，旧 `record_only` 任务继续只记录。
- 群内有效 `@` 继续进入原群 Jarvis，检查它保留原有 Agent、技能、工具、记忆范围、回复身份和确认通道，不继承 Owner 私聊权限，也不把群结果转到 Owner 私聊。
...
- 已完成机器人交互任务在投递过程中中断：投递及其尝试恢复为 unknown，任务不重新执行，也不盲目再次发送；新的 Owner proactive 通知按幂等键恢复，历史 `record_only` 不创建通知。
