...
状态：本地管理台、记忆卡片、任务执行记录及 YAML Agent 编辑、删除已在源码实现；记忆查询只读。用户体验、范围和实施顺序以[主文档](local-console-design.md)为准；[使用说明](../guides/local-console-user-guide.md)说明实际入口。统一服务重启、失败任务继续和只读过程输出 SSE 已实现；任务取消与 Desktop 仍为后续设计。
...
### relayer-next 复用与开发约定

采用 React + TypeScript + Vite 替代此前“原生 JS、无前端构建”方案，便于复用 relayer-next 的 React 卡面及阅读布局；Vue 技术上也可承接，但本次没有选择 Vue。`MemoryCollectibleCard`、`CardDetailView` 适配到现行六类 Memory，删除费用、稀有度、热度、升级和发现标记依赖；MIT 许可随源码及内嵌资源保留。
...
页面路径为 `/tasks`、`/memories`、`/running`、`/settings`，服务器仅对这四个路径及其末尾斜杠形式返回入口 HTML，未知路径仍返回 404。资源和 API 使用根相对地址，直接访问及刷新不受当前路径影响。前端启动从 pathname 选择页面，根地址和 `index.html` 以 replaceState 归一到 `/tasks`；侧栏普通点击用 pushState，popstate 恢复页面并使旧请求失效，不重复写历史。导航采用实际链接，保留键盘、复制地址及修饰键新标签打开；当前页标记 aria-current，标题随页面更新。URL 暂不保存筛选、分页或详情。
...
任务记录、执行实例、采集连接和交付结果分别展示。任务详情的 `deliveries` 使用 `purpose` 与 `transport` 区分交互答复、接收回执、处理中标记、完成标记和失败回执，分别呈现“已收到”“处理中”“已完成”“打叉”的平台结果；私聊与群 @ 阶段历史都保留在管理台，原消息上只保留当前标记。接收或阶段标记被平台接受不能解释为最终回答已经送达。新群任务的操作详情与确认入口合入原群答复；模板配置后使用所有者按钮确认卡片，无模板暂保留原群口令，旧私聊确认记录保留历史分类。首页“需要关注”是展示分组，不新增任务状态或改写数据库。
...
正文保留规则必须经过所有管理台查询入口统一执行，不能直接把旧的详情结构无过滤返回。检查范围包括任务请求、列表预览、尝试结果、日志摘录和记忆引用。对于来源过期的关联正文，按现行治理规则隐藏；对无法安全确认的历史自由文本副本，首版仅提供元数据与错误码。
...
删除检查当前 YAML 和已应用配置的所有应用与群绑定引用，包括停用但保留的配置引用。未绑定 Agent 可以删除；删除不涉及任务、记忆、Preset 和技能目录。默认运行时未声明的 Agent 不支持此编辑接口。
...
React `Time` 组件和原 DOM 适配器的 `time.js` 共享相对时间格式：少于一分钟显示“刚刚”，未来的短间隔显示“即将”，其余按分钟、小时、天、月、年显示前/后。缺失显示未记录，非法值显示时间未知；没有结果的结束时间仍显示尚未记录。datetime 保留原始时刻的 ISO 表示，title 与 aria-label 包含完整日期和时区；可见页面每 15 秒更新已有时间节点，避免重绘列表破坏浏览位置。任务、尝试、投递、消息、诊断日志、终端、记忆卡面与历史、有效期、心跳、数据源覆盖及采样时间均使用同一格式，原文中的自由文本不改写。
...
早期增量使用原生 DOM/CSS 吸收卡面质感及正文优先的详情体验；当前已迁入 React/TypeScript 前端，复用范围见“技术组织”。没有迁入套牌、角色或战斗模型。`Memory`、`Source`、版本与任务记录仍存储在原有 SQLite 表，无数据迁移。

### 只读接口

接口沿用管理台本机访问、同源与 `no-store` 策略，查询在只读事务中执行：

| GET 路径（前缀 `/api/v1`） | 行为 |
| --- | --- |
| `/memory-workspaces` | 返回可选工作区 ID 与名称；全局和全部工作区由 UI 明确提供 |
| `/memories` | `workspace` 默认 global；`all_workspaces=true` 显式查询全部；`category`、`status`（默认 active，支持 all）、`q`、`page` 筛选 |
| `/memories/{id}` | 读取当前 Memory；可通过正整数 `version` 读取历史版本 |
| `/memories/{id}/sources` | 读取该记忆当前或指定版本引用的来源，重新核对原文可用性 |
| `/memories/{id}/history` | 返回历史 Memory 及对应操作记录，每个版本的证据引用均重新核验 |

列表按 `updated_at DESC,id ASC` 稳定排序，固定每页 24 条，返回 cards、total、page、page_size。总数与列表使用同一过滤条件和事务；页码越界收敛到末页。搜索为标题、摘要、正文的字面子串，查询最长 500 字；百分号与下划线不作为通配符。详情、来源、历史接口都要求明确工作区，不能通过 `all_workspaces` 绕过单项范围检查；全部工作区列表点击后携带该条记忆的实际工作区。

卡面只读展示摘要前 100 个 Unicode 字符；全文不修改。详情在独立原生 dialog 中，关闭恢复触发卡片焦点和列表位置。来源与版本历史展开后才加载；可见页沿用 3 秒刷新，结果未变化时不重绘。标签页隐藏、断线、切换页面后清空敏感数据，来源的未来到期时间驱动本地清空和重新查询；不会将全文写入 localStorage/IndexedDB。

记忆正文支持标题、段落、列表、引用、代码块、行内代码、粗体、斜体、分隔线和外链。仅用 DOM 文本节点构造内容，原始 HTML 显示为文本；复杂嵌套或其他未支持 Markdown 语法保持可读原文。仅允许 http、https、mailto 外链，图片显示说明及主动打开链接，不自动请求图片。无需前端构建或第三方渲染运行时。

来源沿用核心 `ReadSource` 和记忆版本读取，并额外核对来源关联消息是否过期、撤回或被新修订替代。不再可用的原文和 evidence.quote 隐藏；正式结论、来源 ID 与可保留的定位继续展示。列表不返回 evidence 引用原文。
...
本次使用独立临时数据库与合成记录验证记忆查阅和任务交付核对，属于“事项闭环”的展示验收，不代表真实模型执行、平台投递或用户业务验收。不会安装二进制或重启真实服务。
