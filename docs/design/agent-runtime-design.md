...
Owner Agents may opt into memgov's own read-only Dokki and Confluence tools. They search a named source and read the relevant document before citing it; source content is evidence and does not become Workspace knowledge automatically. The analyzer and group Agents do not receive these tools. Credentials are kept in memgov's private data home, and the Agent runtime no longer needs Relayer. [The runtime guide](../guides/runtime-user-guide.md#owner-knowledge-sources) shows the setup; [the detailed design](agent-runtime-design-detail.md#owner-knowledge-mcp) defines the tool boundary and legacy migration.
