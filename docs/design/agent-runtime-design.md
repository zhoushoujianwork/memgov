...
Owner Agents may opt into read-only Dokki and Confluence search through a dedicated Relayer MCP connection. They search a named source and read the relevant document before citing it; source content is evidence and does not become Workspace knowledge automatically. The analyzer and group Agents do not receive this connection. Credentials and live access are configured in Relayer; [the runtime guide](../guides/runtime-user-guide.md#owner-knowledge-sources) shows the setup, and the [detailed design](agent-runtime-design-detail.md#owner-knowledge-mcp) defines the tool boundary.
