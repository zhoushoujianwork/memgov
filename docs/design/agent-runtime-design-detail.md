...
An Agent declaration may set `knowledge_mcp.sources` to `dokki`, `confluence`, or both, including Agents selected by `group_mention.default_agent` and exact group bindings. Read-only MCP access does not require Bash. Agents without the declaration receive no knowledge MCP tools, and the analyzer receives no MCP tools; group attempts still receive the separate bot MCP described below. The connection does not inherit the Codex app's MCP configuration or credentials. A sources-only declaration starts the current memgov executable as a private stdio MCP server. Existing declarations with an absolute `command` and read-only `args` remain accepted for rollback, but the built-in path has no Relayer runtime dependency.

At execution, memgov passes selected sources as `memgov_knowledge` through Claude's `--strict-mcp-config`. `dontAsk` permissions admit the selected source's list/search/exact-read operations: Dokki `list_dokki_workspaces`, `search_dokki`, `read_dokki_resource`; Confluence `search_confluence`, `read_confluence_page`, plus advanced read-only `list_confluence_tools`, `query_confluence`, `read_confluence_resource`. The knowledge server has no write or messaging tools. Group attempts additionally receive the separate `memgov_bot` MCP with exactly `resolve_bot_user`, `resolve_bot_group` and `forward_bot_message`; it is bound to the claimed task and attempt and sends directly through the application bot after recording a durable idempotency key. Prompt instructions govern when the Agent uses it; recipient resolution and mounted-group checks remain in code. The analyzer, proactive Agent and Owner private Agent receive no bot MCP. Dokki uses its fixed REST endpoint. Common Confluence page search and exact reading use its REST endpoint with the personal PAT; advanced read tools still use the fixed MCP Dock endpoint and a local allowlist. Each knowledge request has a 20-second deadline and a 16 MiB response limit, and remote errors cannot echo credentials. The Agent is told to cite the human-facing source URL and to report an unconfigured source or failed read. Source text never authorizes actions or automatic Workspace writes. A group Agent's answer goes to its triggering group, where all members may see cited content. Adding a source or switching server modes is a permission/boundary change in `config plan`; the direct native session policy digest also changes.

The MCP `tools/list` schemas encode `required` as an array, including an empty array for tools with no required arguments. Claude rejects a `null` value and otherwise connects to the server without exposing any tools. An isolated group-mode Claude check must verify actual MCP tool dispatch, not only server connection or a successful direct source read.

`memgov knowledge import-relayer --from /absolute/path/to/external-knowledge.sqlite` is an explicit one-time migration. It reads the legacy SQLite file without changing it and writes `knowledge-sources.json` under the memgov home as a mode-0600 regular file. It requires both configured sources, does not print credentials, and succeeds again only when the destination already contains the same values. `memgov knowledge configure --input <private JSON file or ->` can replace credentials directly without Relayer; `memgov knowledge status` reports configuration booleans only. Back up the memgov credential file with the data home; rollback can use the preserved legacy file and legacy command declaration. Source contents are never copied into Agent Workspace knowledge.
...
Group applications also accept `execution_timeout_seconds` in both YAML forms. Omission preserves the existing 900-second default, and explicit integers from 1 to 86400 are accepted; 3600 gives one hour. Apply passes the duration into durable runtime scheduling, so later applies do not reset a chosen duration. New task and confirmed-action leases derive their absolute deadline from that duration. Existing leases retain their original deadlines. Stop the managed service before applying a group change. This extends the execution budget; it does not fix an initialization step that fails to observe cancellation or prove a stalled request will finish.
