...
状态：CLI 和 Web 入口已在源码实现；本次私聊原生会话续接需要更新程序并显式升级到 Schema 26。本地安装和真实钉钉运行验收以交付说明为准。实现与验证见[详细稿](task-continuation-detail.md)。
...
- 同一钉钉 Owner 私聊也绑定独立的原生 Agent 会话；服务重启后在策略和已接受历史未变化时自动 resume，变化或会话不可用时安全回放历史，不跨会话复用。
