...
`config show` 显示解析后的系统默认值、`runtime_setup`、`logging` 和 `channels` 声明，不解析密钥引用。`config validate` 离线检查字段、系统参数与通道身份；路由策略和工作区在应用事务中校验。两者均不初始化数据库。未知字段、重复键、多文档 YAML 和错误类型返回 `invalid_input`。
...
`config plan` 预览双模式 YAML 的声明、已应用版本、披露边界及自动挂载，`config apply-runtime` 绑定计划摘要与版本执行应用；保存 YAML 不等于配置已生效。具体输入与授权沿用[接入详细稿](../design/dingtalk-integration-design-detail.md#双模式-yaml-声明与受限应用)。
...
`runtime configure --input -` 输入 name、channel、route_ids 和 owner，可选 Agent 策略、claude_profile、模型、preset 与调度参数。proactive 不需要 delivery_route_id，completion_policy 派生为 record_only；direct/group_mention 派生为 reply_to_trigger，并核验对应私聊/原群出站路由。group_mention 的 context_channel 可省略；提供时仍要求同企业同群。owner 必须是 DWS 已验证稳定身份。修改已有配置需要 expected version，running 状态不能修改。claude_profile 只读取受支持的本机 alias 环境配置，模型 profile 跟随该配置。
...
DWS 来源独立采集并评估后台事项，值得处理即可启动独立 Agent；成功、失败、clarification、awaiting_confirmation 均只记录，不生成自动完成通知。机器人仅已核验 Owner 私聊及有效群 @ 直接进入 Agent、不走 Haiku。两种交互任务都以独立 Outbox 依次记录“已收到”“处理中”“已完成”或“打叉”；阶段平台调用在有序旁路队列执行，不作为 Agent 启动前置条件。`awaiting_confirmation` 保持处理中，服务启动按任务真相补齐未开始的阶段。失败或外部操作结果未知的群任务若已有非空任务结果、摘要或动作结果，还会以 `runtime_failure_notice` 幂等回复原群；没有结果时不生成空洞回复。群回答 @ 发起人，待确认操作只由该群内 Owner 批准；Owner 发起群请求仍使用群策略。普通用户私聊拒绝进入 Agent。旧 proactive 待发通知失效，不能通过发送/重试恢复。
...
`runtime message send <task-id> --input <JSON-file>` 在当前 running proactive task/attempt、owner_delegated 策略与 DWS Owner 身份下执行独立沟通；`runtime message list <task-id>` 查询审计。输入包含 attempt_id、idempotency_key、target_type（group/user）、稳定原生 target_id、content、reason、evidence_message_ids。目标、实际披露证据及当前配置均复核；固定绑定 profile、本人与 AI 标识。同键同正文返回已有记录，改正文重用键冲突；失败或未知也不盲目重发。结果记录于 runtime_message_actions，和完成通知分离。完整协议见[独立沟通工具](../design/dingtalk-integration-design-detail.md#独立沟通工具)。本轮源码实现、离线验证已通过，未安装、未真实发送。
