# Personal Jarvis 最佳落地场景与对齐标准

状态：Personal Jarvis 产品主线与后续业务验收基准。目标行为不等同于已安装或已在真实平台验收的能力；逐项证据见[实现状态](../implementation-status.md)和[详细稿](best-practice-scenarios-detail.md)。
...
| 场景 | 用户期待的体验 | 完成标准 |
| --- | --- | --- |
| Personal Jarvis：替用户接住工作 | 用户从任意平台一句话即可提问、交办、跟进；本地发现的事项也能继续调查，复杂事项由 Agent 分工完成 | 及时回执；上下文和权限可解释；结果、阻塞、确认和证据可追溯；不重复执行未知外部动作 |
| 本地主动值守：主动推进用户的事项 | 不必把所有背景先整理齐，助手可在授权范围内调查和执行；有实质结果时回原入口通知用户 | 观察与执行任务隔离；历史缺口显式说明；无变化不打扰 |
| 群挂载 Jarvis：团队继续使用群助手 | 群成员在原群 @ 已挂载的 Jarvis，继续使用该群原有的人设、资料、工具、技能和记忆 | Personal Jarvis 重构不改变群 Jarvis 的接入通道、能力或回复路由；群消息不自动取得用户私聊历史和本人身份；未来限制另行决定 |
