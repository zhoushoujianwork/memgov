...
- Group bot forwarding now has source-level MCP tools for recipient resolution and an Owner-confirmed send. Next, verify one real group request, exact recipient resolution, confirmation in that group, and the application bot's delivery receipt after an authorized installation. Keep unknown send outcomes as unknown; do not retry them automatically.
