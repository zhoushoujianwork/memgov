...
- Owner and group Agents can be configured with memgov's read-only Dokki/Confluence tools. The current local configuration grants both sources to each declared Agent, including the default group Agent and its exact binding; source search and exact read passed before this group expansion. Next, verify real Owner and group answers with citations and check that group audiences receive only intended source content. Tool-level checks do not establish a full Agent business outcome.
