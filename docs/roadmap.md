...
- Owner-only Dokki/Confluence tools now run from the installed memgov binary with credentials in its private home. Dokki live search and exact read passed without a Relayer process. Investigate the remote Confluence search rejection and then verify exact Confluence reading and one real Owner Agent answer with a citation. Tool-level checks do not establish a full Agent business outcome.
