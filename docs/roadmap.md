...
- Group bot forwarding passed one live group-to-Owner-DM test: exact recipient resolution, application-bot acceptance, DWS DM search and a final group report without confirmation. Next, test another same-tenant recipient and a mounted-group target when those audiences are explicitly authorized. Keep unknown send outcomes as unknown; do not retry them automatically.
