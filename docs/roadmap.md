...
- Owner-only Dokki/Confluence tools now run from memgov with credentials in its private home. Dokki live search and exact read passed; Confluence page search and exact read passed through direct REST after the advanced Dock search rejected valid requests. Next, verify one real Owner Agent answer with a citation. Tool-level checks do not establish a full Agent business outcome.
