...
func TestRuntimeAgentResolvesExactGroupBindingAndLivePolicy(t *testing.T) {
	...
	policies := map[string]RuntimeAgentPolicy{"default": {Preset: "default-preset", ExternalActions: "owner_confirmation", Capabilities: []string{}}, "special": {Preset: "special-preset", ClaudeProfile: "cc", ExecutionModel: "profile", ExternalActions: "owner_confirmation", Capabilities: []string{"local_read"}, Directories: []string{"/explicit/group/docs"}, KnowledgeMCP: &RuntimeKnowledgeMCP{Sources: []string{"dokki", "confluence"}}}}
	...
	for _, tt := range []struct{ route, agent, preset string }{{one.ID, "special", "special-preset"}, {two.ID, "default", "default-preset"}} {
		...
		if err != nil || p.Agent != tt.agent || p.Preset != tt.preset || (p.KnowledgeMCP != nil) != (tt.route == one.ID) {
