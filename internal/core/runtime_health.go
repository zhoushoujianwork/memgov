...
type RuntimeWorkStatus struct {
	...
	ReviewActive         int    `json:"review_active"`
...
func readRuntimeWorkStatuses(ctx context.Context, q Queryer) (map[string]RuntimeWorkStatus, error) {
 ...
 (SELECT count(*) FROM runtime_work_leases WHERE released=0 AND phase='review'),
 ...
 (SELECT count(*) FROM runtime_attempts a JOIN runtime_tasks t ON t.id=a.task_id WHERE t.runtime_id=c.id AND a.error_code IN ('execution_timeout','review_timeout')),
 ...
 (SELECT count(*) FROM runtime_tasks WHERE runtime_id=c.id AND (status IN ('failed','blocked','clarification','awaiting_confirmation','action_unknown','action_failed','stale') OR memory_status IN ('failed','rejected'))),
 (SELECT count(*) FROM runtime_tasks WHERE runtime_id=c.id AND status='running'),
 (SELECT count(*) FROM runtime_reviews WHERE runtime_id=c.id AND status='pending')
 FROM runtime_configs c`)
	...
	for rows.Next() {
		...
		if err = rows.Scan(&id, &maxWait, &s.AnalysisActive, &s.ExecutionActive, &s.ReviewActive, &s.QueuedTasks, &s.OldestWaitingAt, &s.LastAnalysisAt, &s.RetryBatches, &s.AnalysisGaps, &s.TimedOut, &s.AnalysisLimit, &s.ExecutionLimit, &blocked, &running, &s.QueuedReviews); err != nil {
			return nil, err
		}
