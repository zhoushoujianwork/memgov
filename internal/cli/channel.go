...
func (a *app) audienceCommands() {
	root := &cobra.Command{Use: "audience", Short: "受众隔离与显式披露"}
	var conversation, reason string
	publish := a.write("publish <channel> <memory-id>", "显式允许向某会话披露某个记忆版本", cobra.ExactArgs(2), func(ctx context.Context, tx *core.Tx, args []string, _ json.RawMessage) (any, error) {
		return tx.Publish(ctx, args[0], conversation, args[1], reason)
	})
	publish.Flags().StringVar(&conversation, "conversation", "", "已绑定的会话 ID")
	publish.Flags().StringVar(&reason, "reason", "", "披露理由（必填）")
	root.AddCommand(publish)
	unpublish := a.write("unpublish <channel> <memory-id>", "撤销披露许可并作废相关草稿", cobra.ExactArgs(2), func(ctx context.Context, tx *core.Tx, args []string, _ json.RawMessage) (any, error) {
		return tx.Unpublish(ctx, args[0], conversation, args[1], reason)
	})
	unpublish.Flags().StringVar(&conversation, "conversation", "", "已绑定的会话 ID")
	unpublish.Flags().StringVar(&reason, "reason", "", "撤销理由（必填）")
	root.AddCommand(unpublish)
	root.AddCommand(a.read("check <channel> <memory-id>", "解释某个记忆能否向该会话披露", cobra.ExactArgs(2), func(ctx context.Context, s *core.Store, args []string) (any, error) {
		audience, err := core.AudienceFor(ctx, s.DB, args[0], conversation)
		if err != nil {
			return nil, err
		}
		return core.CheckDisclosure(ctx, s.DB, audience, args[1])
	}))
	var budget int
	var explain bool
	recall := a.read("recall <channel> <question>", "在该会话受众范围内召回", cobra.ExactArgs(2), func(ctx context.Context, s *core.Store, args []string) (any, error) {
		audience, err := core.AudienceFor(ctx, s.DB, args[0], conversation)
		if err != nil {
			return nil, err
		}
		return core.AudienceRecall(ctx, s.DB, audience, args[1], budget, explain)
	})
	...
	var limit int
	list := a.read("list <channel>", "按更新时间列出该会话可查的最新记忆", cobra.ExactArgs(1), func(ctx context.Context, s *core.Store, args []string) (any, error) {
		audience, err := core.AudienceFor(ctx, s.DB, args[0], conversation)
		if err != nil {
			return nil, err
		}
		return core.ListAudienceMemories(ctx, s.DB, audience, limit)
	})
	list.Flags().IntVar(&limit, "limit", 1, "条数（1..20）")
	root.AddCommand(list)
	root.AddCommand(a.read("show <channel> <memory-id>", "读取该会话可查的记忆正文", cobra.ExactArgs(2), func(ctx context.Context, s *core.Store, args []string) (any, error) {
		audience, err := core.AudienceFor(ctx, s.DB, args[0], conversation)
		if err != nil {
			return nil, err
		}
		return core.ReadAudienceMemory(ctx, s.DB, audience, args[1])
	}))
	for _, cmd := range root.Commands() {
		if cmd.Flags().Lookup("conversation") == nil {
			cmd.Flags().StringVar(&conversation, "conversation", "", "已绑定的会话 ID")
		}
	}
	a.root.AddCommand(root)
}
