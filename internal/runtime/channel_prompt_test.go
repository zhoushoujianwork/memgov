...
func TestDingTalkChannelSystemPromptsKeepConversationBoundaries(t *testing.T) {
	...
	for _, expected := range []string{"group mention", "same-group history", "application bot identity", "Do not call dws", "never use DWS --as user", "even when the owner is the requester", "bot-scoped tool", "pending action", "Do not inspect a member's private chat"} {
