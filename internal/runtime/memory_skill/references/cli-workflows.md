...
In the owner-private runtime, the installed `memgov` command is a controlled
wrapper. It binds the current home, workspace and audit actor automatically.
Omit `--home`, `--workspace`, `--actor` and `doctor` from the generic examples
below. Memory writes require the Agent's `local_write` capability; their input
file must be inside the direct session directory and carry an idempotency key.
Configuration, governance, cross-workspace access and backup operations are not
